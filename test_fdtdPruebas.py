...
class FDTD1D():
    def __init__(self, xE, pi_subgrid, pf_subgrid,  boundary, relative_epsilon_vector=None):
        self.xE = xE
        self.xH = (xE[1:] + xE[:-1]) / 2.0
        self.xsubE = []
        for i in range(pi_subgrid,pf_subgrid):
            step1 = (xE[i+1]-xE[i])/4.0
            self.xsubE.append(xE[i]+step1)
            self.xsubE.append(xE[i+1]-step1)
